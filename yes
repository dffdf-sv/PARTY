...
<head>
    ...
    <title>🎉 Party Invite | With Google Drive</title>
    <style>
        * {
            margin: 0;
            padding: 0;
        ...
        body { color: #fff; overflow-x: hidden; position: relative; }
        ...
        .hero h1 {
            ...
            margin-bottom: 1.5rem; text-shadow: 0 0 20px rgba(0,0,0,0.6);
            animation: pulse 2s infinite alternate;
        }
        @keyframes pulse {from{transform:scale(1);}to{transform:scale(1.05);}}
        .hero p.tagline {font-size: clamp(1.2rem,3vw,2rem); margin-bottom:3rem; max-width:800px;}

        /* Buttons */
        .btn {
            padding:1rem 2.5rem; font-size:1.2rem; border:none;
            border-radius:50px; background:#fff; color:#e91e63;
            font-weight:bold; cursor:pointer; transition:all 0.3s ease;
            box-shadow:0 8px 20px rgba(0,0,0,0.3);
            text-decoration:none; display:inline-block; margin:0.5rem;
        }
        .btn:hover {transform:translateY(-5px); background:#222; color:#fff;}
        .btn-admin {background:#ffc107; color:#000;}
        .btn-admin:hover {background:#b78900; color:#fff;}
        .btn-drive {background:#4285F4; color:#fff;}
        .btn-drive:hover {background:#3367D6; color:#fff;}
        ...
        .count-box {
            background:rgba(0,0,0,0.4); padding:1rem 1.5rem;
            border-radius:10px; min-width:80px; backdrop-filter:blur(8px);
        }
        .count-box span {font-size:2.5rem; font-weight:bold; display:block;}

        /* Sections */
        .section-title {text-align:center; font-size:2.2rem; margin:4rem 0 2rem;}
        .details {
        ...
        .drive-box {
            ...
            background:rgba(66, 133, 244, 0.2); padding:2rem;
            border-radius:15px; border:1px solid rgba(66,133,244,0.4);
        }

        /* Admin Portal */
        .admin-login,.admin-panel {
            max-width:700px; margin:3rem auto; padding:2rem;
            background:rgba(0,0,0,0.7); border-radius:15px; display:none;
        }
        ...
        .admin-form input,.admin-form textarea {
            width:100%; padding:1rem; margin:0.5rem 0 1.5rem;
...
<div class="drive-box" id="driveSection" style="display:none;">
    <h3>📁 View Party Files / Photos / Info</h3>
    <a id="driveLink" href="#" target="_blank" class="btn btn-drive">Open Google Drive Folder</a>
</div>

<!-- Admin Login & Edit Panel -->
<div class="admin-login" id="adminLoginBox">
    <h2 style="text-align:center;margin-bottom:1rem;">Admin Login</h2>
    <input type="password" id="adminPass" placeholder="Password: 1234">
    <button class="btn" onclick="adminLogin()">Login</button>
</div>

<div class="admin-panel" id="adminPanelBox">
    <h2 style="text-align:center;margin-bottom:1rem;">✏️ Edit Party & Google Drive</h2>
    <form class="admin-form" onsubmit="saveEdits(event)">
        ...
        <label>Party Date (YYYY-MM-DD HH:MM)</label>
        <input type="text" id="edit-partydate" placeholder="2026-08-22 16:00">
        
        <label>🔗 Google Drive Folder Link</label>
        <input type="url" id="edit-drivelink" placeholder="Paste your Google Drive folder/file link here">
...
const DEFAULT_DATE = "2026-08-22 16:00";
...
window.onload = function() {
    ...
    if(saved) {
        ...
        if(saved.driveLink) {
            document.getElementById('driveLink').href = saved.driveLink;
            document.getElementById('driveSection').style.display = "block";
        }
        startCountdown(saved.partyDate || DEFAULT_DATE);
    } else {
        startCountdown(DEFAULT_DATE);
    }
};
...
function adminLogin() {
    if(document.getElementById('adminPass').value === "1234") {
        ...
        document.getElementById('edit-drivelink').value = localStorage.getItem('partyContent') ? JSON.parse(localStorage.getItem('partyContent')).driveLink || "" : "";
...
function saveEdits(e) {
    ...
    const driveLink = document.getElementById('edit-drivelink').value.trim();
    ...
    const content = {title, tagline, dateText, locText, extraText, partyDate, driveLink};
    localStorage.setItem('partyContent', JSON.stringify(content));

    if(partyDate) startCountdown(partyDate);
    alert("✅ Saved! Google Drive link added!");
}
